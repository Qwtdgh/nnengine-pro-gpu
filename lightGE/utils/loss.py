...
def crossEntropyLoss(pred: Tensor, target: Tensor) -> Tensor:
    ...
    # return (-target * pred.log()).sum(axes=None) / Tensor(np.array(pred.shape[0]), autograd=False)
    # pred加一个极小数防止除0
    return -target * (pred + Tensor(np.array(1e-300), autograd=False)).log()
...
def multi_classification_cross_entropy_loss(pred: Tensor, target: Tensor) -> Tensor:
    ...
    return (-target * pred.log()).sum((0, 1, 2)) / Tensor(np.array(sum(tgt_len)), autograd=False)
...
def multi_classification_kld(pred: Tensor, target: Tensor) -> Tensor:
    ...
    for i in range(target.data.shape[0]):
        ...
        if zero_rows.size == 0:
            ...
            tgt_len.append(zero_rows[0])
    return (-target * (target / pred).log()).sum((0, 1, 2)) / Tensor(np.array(sum(tgt_len)), autograd=False)
