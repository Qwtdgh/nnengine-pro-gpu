...
from lightGE.core.transformer.encoder import Encoder
from lightGE.core.transformer.decoder import Decoder
...
# Transformer模型
from lightGE.data.segmentation.segmentation import add_start_end_token


class Transformer(Model):
    def __init__(self, encoder_block=6, decoder_block=6, batch=8, sentence_len=5, n_inputs=512, n_heads=2,
                 hidden_feedforward=2048, vocab_len=1000, ndrop_prob=0.9):
        """
        ...
        self.encoder = Encoder(block_num=encoder_block, batch=batch, sentence_len=sentence_len, n_inputs=n_inputs,
                               n_heads=n_heads, hidden_feedforward=hidden_feedforward, ndrop_prob=ndrop_prob)
        self.decoder = Decoder(block_num=decoder_block, batch=batch, sentence_len=sentence_len, n_inputs=n_inputs,
                               n_heads=n_heads, hidden_feedforward=hidden_feedforward, ndrop_prob=ndrop_prob)
        self.linear = Linear(n_inputs, vocab_len)
    ...
    def predict(self, inputs, **kwargs) -> str:
        ...
        tgt = Tensor(tgt_array, autograd=False)
        tgt_len = [1]
...
def predict(english, sentence_loader, model: Model):
    model.eval()
    ...
    src = Tensor(np.array([[sentence_loader.src_word2vec.wv[tokens[i]].tolist()
                          ...
                          ).reshape(1, sentence_loader.max_sentence_len, -1), autograd=False)
